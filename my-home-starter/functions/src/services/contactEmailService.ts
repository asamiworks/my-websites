...
export class ContactEmailService {
  ...
  async sendContactEmail(data: {
    ...
    try {
      ...
      await this.sendEmail(
        functions.config().gmail.admin_email || functions.config().gmail.sender_email,
        `【お問い合わせ】${name}様より`,
        this.createAdminEmailHtml(name, email, message, timestamp),
        this.createAdminEmailText(name, email, message, timestamp)
      );

      // ユーザーへの自動返信メール
      await this.sendEmail(
        email,
        "お問い合わせありがとうございます - マイホームスターター",
        this.createUserEmailHtml(name, message, timestamp),
        this.createUserEmailText(name, message, timestamp)
      );
   ...
   * 管理者向けメールのHTML本文を生成
   */
  private createAdminEmailHtml(name: string, email: string, message: string, timestamp: string): string {
    return `
<!DOCTYPE html>
<html lang="ja">
<head>
...
<body>
   ...
   * 管理者向けメールのテキスト本文を生成
   */
  private createAdminEmailText(name: string, email: string, message: string, timestamp: string): string {
    return `
新規お問い合わせを受け付けました。
...
このメールはマイホームスターターのお問い合わせフォームから自動送信されました。
   ...
   */
  private createUserEmailHtml(name: string, message: string, timestamp: string): string {
    return `
<!DOCTYPE html>
<html lang="ja">
<head>
...
<body>
   ...
   */
  private createUserEmailText(name: string, message: string, timestamp: string): string {
    return `
${name} 様
