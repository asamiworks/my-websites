// scripts/importAllZEHBuildersFixed.ts
...
class ZEHBuildersAllImporter {
   ...
   * 1つの都道府県ファイルを処理
   */
  private async importPrefectureFile(dataDir: string, filename: string, prefecture: string): Promise<void> {
   ...
   * バッチ処理
   */
  private async processBatch(records: ZEHBuilderCSV[], prefecture: string): Promise<void> {
   ...
   * CSVレコードをパース
   */
  private parseRecord(record: ZEHBuilderCSV, prefecture: string): ParsedRecord | null {
    ...
    const zehPerformance: ZEHPerformance[] = [];
    ...
    const targetPrefectures = this.parseTargetPrefectures(getValue(['対応可能エリア']));
    
    // 現在処理中の都道府県も対応エリアに含める
    ...
    return {
      ...
      businessTypes: this.parseBusinessTypes(getValue(['ZEHビルダー/プランナーZEH普及対象', '種別', '業態', '事業タイプ'])),
   ...
   * 対応可能エリアをパース（セミコロン区切り）
   */
  private parseTargetPrefectures(areaString: string): string[] {
    if (!areaString || areaString === '-' || areaString === '－') {
      return [];
    }
    
    // セミコロンで分割して都道府県名の配列を返す
    return areaString
      .split(';')
      .map(area => area.trim())
  ...
  private parseBusinessTypes(value: string): string[] {
    if (!value) return ['新築注文戸建住宅'];
    
    const types = value.split(/[、,・]/).map(t => t.trim()).filter(Boolean);
    return types.length > 0 ? types : ['新築注文戸建住宅'];
  }

  /**
   * データベースに保存
   */
  private async saveToDatabase(data: ParsedRecord, prefecture: string): Promise<'new' | 'updated' | 'error'> {
    try {
      // キャッシュから会社を検索
   ...
   * 新規会社作成
   */
  private async createCompany(data: ParsedRecord) {
    // 法人番号を取得（オプション）
    ...
    // 対応可能エリアも登録
    for (const targetPref of data.targetPrefectures) {
      if (targetPref !== data.prefecture) {
        try {
          await prisma.serviceArea.create({
            data: {
              companyId: company.id,
   ...
   * 既存会社更新
   */
  private async updateCompany(companyId: string, data: ParsedRecord): Promise<void> {
    ...
    // 対応可能エリアも追加
    for (const targetPref of data.targetPrefectures) {
      const hasTargetArea = existingCompany.serviceAreas.some(
        area => area.prefecture === targetPref
      );
   ...
   * 最終サマリーを表示
   */
  private printFinalSummary(elapsedTime: number): void {
    console.log('\n\n');
    console.log('╔═══════════════════════════════════════════════════════╗');
