...
/**
 * 毎日午前9時に実行され、発行日が今日の請求書のPDFを自動生成
 * Cloud Schedulerで毎日 9:00 (JST) に実行
 */
export const autoGenerateInvoicePDFs = onSchedule({
  schedule: '0 9 * * *',
  timeZone: 'Asia/Tokyo',
  timeoutSeconds: 540,
  memory: '2GiB'
}, async (event): Promise<void> => {
  console.log('Starting automatic invoice PDF generation...');
  ...
  try {
    ...
    const invoicesSnapshot = await db.collection('invoices')
      .where('issueDate', '>=', admin.firestore.Timestamp.fromDate(today))
      .where('issueDate', '<', admin.firestore.Timestamp.fromDate(tomorrow))
      .get();

    if (invoicesSnapshot.empty) {
      console.log('No invoices with issue date today');
      return;
    }
    ...
    for (const invoiceDoc of invoicesSnapshot.docs) {
      ...
      try {
        ...
        if (invoice.pdfUrl) {
          console.log(`Skipping invoice ${invoice.invoiceNumber}: PDF already generated`);
          skipCount++;
          continue;
        }
        ...
        console.log(`Successfully generated PDF for invoice ${invoice.invoiceNumber}: ${fileUrl}`);
        successCount++;
    ...
    console.log(`Automatic PDF generation completed:`);
...
/**
 * 手動でPDF生成をトリガーするHTTP関数（テスト用）
 */
export const triggerAutoPDFGeneration = onRequest(async (request, response) => {
  // 簡易的な認証チェック
  ...
  try {
    console.log('Manual trigger: Starting automatic PDF generation...');
    ...
    if (invoicesSnapshot.empty) {
      response.json({ message: 'No invoices with issue date today' });
      return;
    }
    ...
    response.json({
      ...
      errorCount,
      results,
      message: 'Automatic PDF generation completed'
    });
